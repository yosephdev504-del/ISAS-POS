...
function renderSettingsProductList() {
  ...
  menu.forEach((item, idx) => {
    ...
    row.innerHTML = `
      <span class="settings-product-name">${item.nombre}</span>
      <span style="color:var(--text-muted);font-size:0.75rem;flex-shrink:0">L</span>
      <input type="number" class="settings-price-input" value="${item.precio}" min="1" step="1" data-idx="${idx}" aria-label="Precio de ${item.nombre}"/>
      <button class="btn-save-price" data-idx="${idx}" title="Guardar precio" aria-label="Guardar precio">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
      </button>
      <button class="btn-delete-product" data-idx="${idx}" title="Eliminar producto" aria-label="Eliminar ${item.nombre}">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
      </button>
    `;
    ...
    row.querySelector(".btn-save-price").addEventListener("click", () => {
      const input = row.querySelector(".settings-price-input");
      ...
      saveMenu(menu);
      renderMenu();
      showToast(`Precio de "${item.nombre}" actualizado a L ${newPrice}`);
    });

    // Also save on Enter key
    row.querySelector(".settings-price-input").addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
